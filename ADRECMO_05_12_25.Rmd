...
```{r Figure1-display, fig.width=8, fig.height=6, fig.scale=1.5, dpi=300, fig.align='center'}
...
# FIGURE 2: Association between ADRB expressions and outcomes

```{r Figure2_tot,include=FALSE}

prep_A1 <- prep_marker_data(df, prefix = "ADR1L", transform = "none")
test_A1 <- test_marker(prep_A1$data)
Figure_2_1 <- plot_marker(
  dat      = prep_A1$data,
...
Figure2 <- (Figure_2_1 + Figure_2_2 + Figure_2_3 + Figure_2_4) +
  patchwork::plot_layout(guides = "collect") &
  theme(legend.position = "bottom")
```

```{r Figure2_tot-display, fig.width=8, fig.height=6, fig.scale=1.5, dpi=300, fig.align='center'}
...
# FIGURE 4: Association between VTI and % of expression of ADRB on monocyte at admission

```{r Figure4,include=F}
# ADRB1

mw_3a <- wilcox.test(log1p(M_ADR1_J0) ~ VTI_JS, data = df)
p_value_VTI <- sprintf("%.3f", mw_3a$p.value)

count_data <- df %>% group_by(VTI_JS) %>% summarise(n = n(), .groups = "drop")

Figure4a <- ggplot(df, aes(x = VTI_JS, y = log1p(M_ADR1_J0), fill = VTI_JS)) +
  geom_boxplot() +
  geom_point(size = 2, alpha = 0.7, position = position_jitter(width = 0.1)) +
  ...
  theme_minimal() + theme(legend.position = "none") +
  annotate("text", x = 1.5, y = max(log1p(df$M_ADR1_J0), na.rm = TRUE), label = paste("p:", p_value_VTI), size = 5)

# ADRB2

mw_3b <- wilcox.test(log1p(M_ADR2_J0) ~ VTI_JS, data = df)
p_value_VTI_2 <- sprintf("%.3f", mw_3b$p.value)

count_data2 <- df %>% group_by(VTI_JS) %>% summarise(n = n(), .groups = "drop")

Figure4b <- ggplot(df, aes(x = VTI_JS, y = log1p(M_ADR2_J0), fill = VTI_JS)) +
  geom_boxplot() +
  geom_point(size = 2, alpha = 0.7, position = position_jitter(width = 0.1)) +
  scale_fill_manual(values = c("≤12 cm" = "#0072B2", ">12 cm" = "#D55E00")) +
  labs(title = "B", x = "VTI measured at 1.5 L/min", y = "% of Monocytes expressing ADRB2 at ECMO weaning \n (log scale)") +
  theme_minimal() + theme(legend.position = "none") +
  annotate("text", x = 1.5, y = max(log1p(df$M_ADR2_J0), na.rm = TRUE), label = paste("p:", p_value_VTI_2), size = 5) +
  geom_text(data = count_data2, aes(x = VTI_JS, y = min(log1p(df$M_ADR2_J0), na.rm = TRUE) * 1.25, label = paste0("n=", n)), inherit.aes = FALSE, size = 4)
```

```{r Figure4_tot-display, fig.width=8, fig.height=6, fig.scale=1.5, dpi=300, fig.align='center'}
...
mw  <- wilcox.test(log_value_IL6_JS ~ VTI_JS, data = df)
p  <- sprintf("%.3f", mw$p.value)
n   <- df %>% count(VTI_JS)

y_max <- max(df$log_value_IL6_JS, na.rm = TRUE)
y_min <- min(df$log_value_IL6_JS, na.rm = TRUE)

Figure5 <- ggplot(df, aes(x = VTI_JS, y = log_value_IL6_JS, fill = VTI_JS)) +
  geom_boxplot() +
  geom_point(size = 2, alpha = 0.7, position = position_jitter(width = 0.1)) +
  scale_fill_manual(values = c("≤12 cm" = "#0072B2", ">12 cm" = "#D55E00")) +
  labs(
    title = "Association between VTI and IL-6 at weaning",
    x = "VTI measured at 1.5 L/min",
    y = "log(IL-6 + 1) at weaning"
  ) +
  theme_minimal() + theme(legend.position = "none") +
  annotate("text", x = 1.5, y = y_max,
           label = paste("p:", p), size = 5, color = "black") +
  geom_text(data = n,
            aes(x = VTI_JS, y = y_min * 1.25, label = paste0("n=", n)),
            inherit.aes = FALSE, size = 4)
```

```{r Figure5-display, fig.width=8, fig.height=6, fig.scale=1.5, dpi=300, fig.align='center'}
...
# FIGURE 7: Survival \~ IL-6 admission

```{r Figure_7_censored,include=FALSE}
...
Figure_7 <- ggsurvplot(km_fit_comp, data = df, 
  ...
  legend.title = " ", palette = c("#E63946", "#457B9D"), ggtheme = theme_bw(),
  legend.labs = c("below median", "above median"), legend.position= "none")

xmax <- max(df$diff_days, na.rm = TRUE)
Figure_7$plot <- Figure_7$plot +
  annotate("text", x = 0.2 * xmax, y = 0.95, hjust = 0, label = lab, size = 3.7)

Figure_7$plot <- Figure_7$plot +
  annotate("text", x = 0.2 * xmax, y = 0.99, hjust = 0, label = paste0("Median IL-6 D0: ", lab2, " pg/ml"), size = 3.7)

Figure_7_full <- ggarrange(
  Figure_7$plot,
  Figure_7$table,
  ncol = 1, nrow = 2,
  heights = c(3, 1)
...
```{r Figure_7_censored-display, fig.width=8, fig.height=6, fig.scale=1.5, dpi=300, fig.align='center'}
...
# FIGURE 8: Association between IL-6 and vasopressor dose

```{r Figure8,include=FALSE}
...
res_J3_J5 <- test_linearity(
  data = df,
  xvar = "log_value_IL6_J3_J5",
  yvar = "jp2_NAd_cum_w"
)
res_J3_J5


res_JS <- test_linearity(
  data = df,
  xvar = "log_value_IL6_JS",
  yvar = "jsev_NAd_cum_w"
)
res_JS

# D0
Figure8_D0 <- make_il6_cor_plot(df,
                                xvar = "log_value_IL6_J0",
                                yvar = "NADcum_D0_w",
                                ylab= "log(Cumulative norepinephrine dose (mg/kg/day)+1)",
                                title_lab = "A: D0")

# J3–J5
Figure8_J3J5 <- make_il6_cor_plot(df,
                                  xvar = "log_value_IL6_J3_J5",
                                  yvar = "jp2_NAd_cum_w",
                                  ylab= "log(Cumulative norepinephrine dose (mg/kg/day)+1)",
                                  title_lab = "B: D3–D5")
...
# JS
Figure8_JS <- make_il6_cor_plot(df,
                                xvar = "log_value_IL6_JS",
                                yvar = "jsev_NAd_cum_w",
                                ylab= "log(Cumulative norepinephrine dose (mg/kg/day)+1)",
                                title_lab = "C: Weaning")
...
```{r Figure8,display, fig.width=8, fig.height=6, fig.scale=1.5, dpi=300, fig.align='center'}

Figure8 <- Figure8_D0 | Figure8_J3J5 | Figure8_JS

Figure8

ggsave("FIGURES/Figure8.pdf",
       plot   = Figure8,
       width  = 240, height = 120, units = "mm",
       device = cairo_pdf, bg = "white")
```

# FIGURE 9: association between IL6 and ECMO flow
...
ggplot(df, aes(x = log_value_IL6_J0, y = j0_ecmo_debit)) +
  geom_point() +
  geom_smooth(method = "lm", colour = "blue", se = FALSE) +       # modèle linéaire
  geom_smooth(method = "loess", colour = "red", se = FALSE) +     # lissage non paramétrique
  theme_classic()
...
res_DW <- test_linearity(
  data = df,
  xvar = "log_value_IL6_JS",
  yvar = "jsev_ecmo_debit"
)
res_DW

F9_plot <- make_il6_cor_plot(df,
                             xvar= "log_value_IL6_J0",
                             yvar="j0_ecmo_debit",
                             ylab="ECMO Flow D0 (L/min)",
                             title_lab = "A: D0")
F9_plot_2 <- make_il6_cor_plot(df,
                             xvar= "log_value_IL6_J3_J5",
                             yvar="jp2_ecmo_debit",
                             ylab="ECMO Flow D0 (L/min)",
                             title_lab = "A: D3-D5")
F9_plot_3 <- make_il6_cor_plot(df,
                             xvar= "log_value_IL6_JS",
                             yvar="jsev_ecmo_debit",
                             ylab="ECMO Flow D0 (L/min)",
                             title_lab = "A: Weaning")
...
```{r Figure9-display, fig.width=8, fig.height=6, fig.scale=1.5, dpi=300, fig.align='center'}

F9_panel <- F9_plot | F9_plot_2 | F9_plot_3

F9_panel

ggsave("FIGURES/Figure9.pdf",
       plot   = F9_panel,
       width  = 160, height = 120, units = "mm",
       device = cairo_pdf, bg = "white")
```

# FIGURE 10: IL6 and duration of Noradrenaline
...
# FIGURE 11: IL6 at D0 and SOFA over-time

```{r Figure11, include=FALSE}

# Construction des 3 panneaux
F11_D0   <- make_il6_sofa_panel(df,
                                il6_var   = "log_value_IL6_J0",
                                sofa_var  = "sofa_D0",
                                panel_label = "A",
                                time_label  = "D0")

F11_J3J5 <- make_il6_sofa_panel(df,
                                il6_var   = "log_value_IL6_J0",
                                sofa_var  = "jp2_sofa",
                                panel_label = "B",
                                time_label  = "D3–D5")

F11_JS   <- make_il6_sofa_panel(df,
                                il6_var   = "log_value_IL6_J0",
                                sofa_var  = "jsev_sofa",
                                panel_label = "C",
                                time_label  = "Weaning day")
```

```{r Figure11-display, fig.width=8, fig.height=6, fig.scale=1.5, dpi=300, fig.align='center'}
# Panel complet Figure 11
Figure11 <- F11_D0 | F11_J3J5 | F11_JS
Figure11

ggsave("FIGURES/Figure11.pdf",
       plot   = Figure11,
       width  = 160, height = 120, units = "mm",
       device = cairo_pdf, bg = "white")
```

# FIGURE 12: Association with T4 cell and outcomes

```{r Figure12, include=F}

df$T4_J0
prep_A1 <- prep_marker_data(df, prefix = "T4", transform = "log1p")
...
Figure12 <- plot_marker(
  ...
  panel_title = "A",
  y_var    = "value_log",
  y_lab    = "Percentage of T4 lymphocyte (log transformed)"
) +
  annotate("text", x = 1, y = min(prep_A1$data$value_log, na.rm = TRUE) +1, label = test_A1$label, hjust = 0)
```
...
# FIGURE 13: Renal function and IL6

```{r Figure13, include=FALSE}
res_D0 <- test_linearity(
  data = df,
  xvar = "log_value_IL6_J0",
    yvar = "CKD_D0"
...
F13_plot <- make_il6_cor_plot(data=df,
                    xvar= "log_value_IL6_J0",
                    yvar = "CKD_D0",
                    ylab = "CKD-EPI (ml/min/1.73m²)",
                    title_lab = "A: D0")

F13_plot_2 <- make_il6_cor_plot(data=df,
                    xvar= "log_value_IL6_J3_J5",
                    yvar = "CKD_J3J5",
                    ylab = "CKD-EPI (ml/min/1.73m²)",
                    title_lab = "B: D3-D5")

F13_plot_3 <-make_il6_cor_plot(data=df,
                    xvar= "log_value_IL6_JS",
                    yvar = "CKD_JS",
                    ylab = "CKD-EPI (ml/min/1.73m²)",
                    title_lab = "C: weaning")
...
```{r Figure13-display, fig.width=8, fig.height=6, fig.scale=1.5, dpi=300, fig.align='center'}

F13_panel <- F13_plot | F13_plot_2 | F13_plot_3

F13_panel

ggsave("FIGURES/Figure13.pdf",
       plot   = F13_panel,
       width  = 160, height = 120, units = "mm",
       device = cairo_pdf, bg = "white")
```

# FIGURE 14: Association IL6 hepatic function

```{r Figure14, include=FALSE}
...
F14_1_plot <- make_il6_cor_plot(data= df,
                   xvar= "log_value_IL6_J0",
                   yvar = "ASAT_D0",
                   ylab= "ASAT (UI/l)",
                   title_lab = "A: D0")

F14_2_plot <- make_il6_cor_plot(data= df,
                   xvar= "log_value_IL6_J3_J5",
                   yvar = "jp2_asat",
                   ylab= "ASAT (UI/l)",
                   title_lab = "B: D3-D5")

F14_3_plot <- make_il6_cor_plot(data= df,
                   xvar= "log_value_IL6_JS",
                   yvar = "jsev_asat",
                  ylab= "ASAT (UI/l)",
                   title_lab = "C: weaning")
...
```{r Figure14-display, fig.width=8, fig.height=6, fig.scale=1.5, dpi=300, fig.align='center'}

F14_panel <- F14_1_plot | F14_2_plot | F14_3_plot

F14_panel

ggsave("FIGURES/Figure14.pdf",
       plot   = F14_panel,
       width  = 160, height = 120, units = "mm",
       device = cairo_pdf, bg = "white")
...
# FIGURE 15: Association IL6 D0 and Other study co-inclusion
...
# FIGURE 16: association lactate and IL6

```{r Figure16, include=FALSE}
...
F16_1_plot <- make_il6_cor_plot(data= df,
                   xvar= "log_value_IL6_J0",
                   yvar = "Lact_D0",
                   ylab= "lactate (mmol/l)",
                   title_lab = "A: D0")

F16_2_plot <- make_il6_cor_plot(data= df,
                   xvar= "log_value_IL6_J3_J5",
                   yvar = "jp2_lact",
                   ylab= "lactate (mmol/l)",
                   title_lab = "B: D3-D5")

F16_3_plot <- make_il6_cor_plot(data= df,
                   xvar= "log_value_IL6_JS",
                   yvar = "jsev_lact",
                  ylab= "lactate (mmol/l)",
                   title_lab = "C: weaning")
...
```{r Figure16-display, fig.width=8, fig.height=6, fig.scale=1.5, dpi=300, fig.align='center'}

Figure16 <- F16_1_plot | F16_2_plot | F16_3_plot

Figure16

ggsave("FIGURES/Figure16.pdf",
       plot   = Figure16,
       width  = 160, height = 120, units = "mm",
       device = cairo_pdf, bg = "white")
```

# FIGURE 17: Association between IL6 and expression of ADRB on T Lymphocytes

```{r Figure17, include=FALSE}

F17_1_plot <- make_il_cor_plot(df, "ADR1L_J0", "log_value_IL6_J0",
                 xlab = "Percentage of T lymphocytes\nexpressing ADRB1",
                 ylab = "log(IL-6 + 1)",
                 title_lab = "A: D0",
                 ylim = c(0, 7))

F17_2_plot <- make_il_cor_plot(df, "ADR1L_J3_J5", "log_value_IL6_J3_J5",
                 xlab = "Percentage of T lymphocytes\nexpressing ADRB1",
                 ylab = "log(IL-6 + 1)",
                 title_lab = "B: D3-D5",
                 ylim = c(0, 7))

F17_3_plot <- make_il_cor_plot(df, "ADR1L_JS", "log_value_IL6_JS",
                 xlab = "Percentage of T lymphocytes\nexpressing ADRB1",
                 ylab = "log(IL-6 + 1)",
...
F17_4_plot <- make_il_cor_plot(df, "ADR2L_J0", "log_value_IL6_J0",
                 xlab = "Percentage of T lymphocytes\nexpressing ADRB2",
                 ylab = "log(IL-6 + 1)",
                 title_lab = "D: D weaning",
                 ylim = c(0, 6))


F17_5_plot <- make_il_cor_plot(df, "ADR2L_J3_J5", "log_value_IL6_J3_J5",
                 xlab = "Percentage of T lymphocytes\nexpressing ADRB2",
                 ylab = "log(IL-6 + 1)",
                 title_lab = "F: D3-D5",
                 ylim = c(0, 6))

F17_6_plot <- make_il_cor_plot(df, "ADR2L_JS", "log_value_IL6_JS",
                 xlab = "Percentage of T lymphocytes\nexpressing ADRB2",
                 ylab = "log(IL-6 + 1)",
                 title_lab = "G: D weaning",
                 ylim = c(0, 6))

Figure_17 <- (F17_1_plot | F17_2_plot | F17_3_plot) /
             (F17_4_plot | F17_5_plot | F17_6_plot)
...
Figure_17
ggsave("FIGURES/Figure17.pdf",
       plot   = Figure_17,
       width  = 240, height = 160, units = "mm",
       device = cairo_pdf, bg = "white")
```

# FIGURE 18: Association between IL6 and expression of ADRB on monocytes

```{r Figure18, include=FALSE}

F18_1_plot <- make_il_cor_plot(df, "M_ADR1_J0", "log_value_IL6_J0",
                 xlab = "Percentage of monocytes\nexpressing ADRB1",
                 ylab = "log(IL-6 + 1)",
                 title_lab = "A: D0",
                 ylim = c(0, 7))

F18_2_plot <- make_il_cor_plot(df, "M_ADR1_J3_J5", "log_value_IL6_J3_J5",
                 xlab = "Percentage of monocytes\nexpressing ADRB1",
                 ylab = "log(IL-6 + 1)",
                 title_lab = "B: D3-D5",
                 ylim = c(0, 7))

F18_3_plot <- make_il_cor_plot(df, "M_ADR1_JS", "log_value_IL6_JS",
                 xlab = "Percentage of monocytes\nexpressing ADRB1",
                 ylab = "log(IL-6 + 1)",
                 title_lab = "C: D weaning",
                 ylim = c(0, 7))

F18_4_plot <- make_il_cor_plot(df, "M_ADR2_J0", "log_value_IL6_J0",
                 xlab = "Percentage of monocytes\nexpressing ADRB2",
                 ylab = "log(IL-6 + 1)",
                 title_lab = "D: D0",
                 ylim = c(0, 6))
...
F18_5_plot <- make_il_cor_plot(df, "M_ADR2_J3_J5", "log_value_IL6_J3_J5",
                 xlab = "Percentage of monocytes\nexpressing ADRB2",
                 ylab = "log(IL-6 + 1)",
                 title_lab = "E: D3-D5",
                 ylim = c(0, 6))

F18_6_plot <- make_il_cor_plot(df, "M_ADR2_JS", "log_value_IL6_JS",
                 xlab = "Percentage of monocytes\nexpressing ADRB2",
                 ylab = "log(IL-6 + 1)",
                 title_lab = "F: D weaning",
                 ylim = c(0, 6))

Figure_18 <- (F18_1_plot | F18_2_plot | F18_3_plot) /
             (F18_4_plot | F18_5_plot | F18_6_plot)
```

```{r Figure18-display, fig.width=8, fig.height=6, fig.scale=1.5, dpi=300, fig.align='center'}
Figure_18
ggsave("FIGURES/Figure18.pdf",
       plot   = Figure_18,
       width  = 240, height = 160, units = "mm",
       device = cairo_pdf, bg = "white")
```

#FIGURE 19: Association between IL10 and expression of ADRB on T Lymphocytes

```{r Figure19, include=FALSE}

F19_1_plot <- make_il_cor_plot(df, "ADR1L_J0", "log_value_IL10_J0",
                 xlab = "Percentage of T lymphocytes\nexpressing ADRB1",
                 ylab = "log(IL-10 + 1)",
                 title_lab = "A: D0",
                 ylim = c(0, 7))

F19_2_plot <- make_il_cor_plot(df, "ADR1L_J3_J5", "log_value_IL10_J3_J5",
                 xlab = "Percentage of T lymphocytes\nexpressing ADRB1",
                 ylab = "log(IL-10 + 1)",
                 title_lab = "B: D3-D5",
                 ylim = c(0, 7))

F19_3_plot <- make_il_cor_plot(df, "ADR1L_JS", "log_value_IL10_JS",
                 xlab = "Percentage of T lymphocytes\nexpressing ADRB1",
                 ylab = "log(IL-10 + 1)",
                 title_lab = "C: D weaning",
                 ylim = c(0, 7))
...
F19_4_plot <- make_il_cor_plot(df, "ADR2L_J0", "log_value_IL10_J0",
                 xlab = "Percentage of T lymphocytes\nexpressing ADRB2",
                 ylab = "log(IL-10 + 1)",
                 title_lab = "D: D0",
                 ylim = c(0, 6))
...
F19_5_plot <- make_il_cor_plot(df, "ADR2L_J3_J5", "log_value_IL10_J3_J5",
                 xlab = "Percentage of T lymphocytes\nexpressing ADRB2",
                 ylab = "log(IL-10 + 1)",
                 title_lab = "E: D3-D5",
                 ylim = c(0, 6))

F19_6_plot <- make_il_cor_plot(df, "ADR2L_JS", "log_value_IL10_JS",
                 xlab = "Percentage of T lymphocytes\nexpressing ADRB2",
                 ylab = "log(IL-10 + 1)",
                 title_lab = "F: D weaning",
                 ylim = c(0, 6))

Figure_19 <- (F19_1_plot | F19_2_plot | F19_3_plot) /
             (F19_4_plot | F19_5_plot | F19_6_plot)
```

```{r Figure19-display, fig.width=8, fig.height=6, fig.scale=1.5, dpi=300, fig.align='center'}

Figure_19
ggsave("FIGURES/Figure19.pdf",
       plot   = Figure_19,
       width  = 240, height = 160, units = "mm",
       device = cairo_pdf, bg = "white")
```

# FIGURE 20: Association between IL10 and expression of ADRB on monocytes

```{r Figure20, include=FALSE}

F20_1_plot <- make_il_cor_plot(df, "M_ADR1_J0", "log_value_IL10_J0",
                 xlab = "Percentage of monocytes\nexpressing ADRB1",
                 ylab = "log(IL-10 + 1)",
                 title_lab = "A: D0",
                 ylim = c(0, 7))

F20_2_plot <- make_il_cor_plot(df, "M_ADR1_J3_J5", "log_value_IL10_J3_J5",
                 xlab = "Percentage of monocytes\nexpressing ADRB1",
                 ylab = "log(IL-10 + 1)",
                 title_lab = "B: D3-D5",
                 ylim = c(0, 7))

F20_3_plot <- make_il_cor_plot(df, "M_ADR1_JS", "log_value_IL10_JS",
                 xlab = "Percentage of monocytes\nexpressing ADRB1",
                 ylab = "log(IL-10 + 1)",
                 title_lab = "C: D weaning",
                 ylim = c(0, 7))

F20_4_plot <- make_il_cor_plot(df, "M_ADR2_J0", "log_value_IL10_J0",
                 xlab = "Percentage of monocytes\nexpressing ADRB2",
                 ylab = "log(IL-10 + 1)",
                 title_lab = "D: D0",
                 ylim = c(0, 6))
...
F20_5_plot <- make_il_cor_plot(df, "M_ADR2_J3_J5", "log_value_IL10_J3_J5",
                 xlab = "Percentage of monocytes\nexpressing ADRB2",
                 ylab = "log(IL-10 + 1)",
                 title_lab = "E: D3-D5",
                 ylim = c(0, 6))

F20_6_plot <- make_il_cor_plot(df, "M_ADR2_JS", "log_value_IL10_JS",
                 xlab = "Percentage of monocytes\nexpressing ADRB2",
                 ylab = "log(IL-10 + 1)",
                 title_lab = "F: D weaning",
                 ylim = c(0, 6))

Figure_20 <- (F20_1_plot | F20_2_plot | F20_3_plot) /
             (F20_4_plot | F20_5_plot | F20_6_plot)
```

```{r Figure20-display, fig.width=8, fig.height=6, fig.scale=1.5, dpi=300, fig.align='center'}
Figure_20
ggsave("FIGURES/Figure20.pdf",
       plot   = Figure_20,
       width  = 240, height = 160, units = "mm",
       device = cairo_pdf, bg = "white")
```

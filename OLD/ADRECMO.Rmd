...
ggsave("FIGURES/Figure1_ADRB_outcomes.pdf",
       plot   = Figure1_tot,
...
mw_VTI1 <- wilcox.test(log1p(ADR1L_J0) ~ VTI_JS, data = F2_1_a)
p_value_VTI <- sprintf("%.3f", mw_VTI1$p.value)

count_data <- F2_1_a %>% group_by(VTI_JS) %>% summarise(n = n(), .groups = "drop")
...
Figure2a <- ggplot(F2_1_a, aes(x = VTI_JS, y = log1p(ADR1L_J0), fill = VTI_JS)) +
  ...
  geom_text(data = count_data,
            aes(x = VTI_JS, y = min(log1p(F2_1_a$ADR1L_J0), na.rm = TRUE) * 1.25, label = paste0("n=", n)),
  ...
  annotate("text", x = 1.5, y = max(log1p(F2_1_a$ADR1L_J0), na.rm = TRUE), label = paste("p:", p_value_VTI), size = 5)
...
mw_VTI2 <- wilcox.test(log1p(ADR2L_J0) ~ VTI_JS, data = F2_2_a)
p_value_VTI_2 <- sprintf("%.3f", mw_VTI2$p.value)

count_data2 <- F2_2_a %>% group_by(VTI_JS) %>% summarise(n = n(), .groups = "drop")
...
Figure2b <- ggplot(F2_2_a, aes(x = VTI_JS, y = log1p(ADR2L_J0), fill = VTI_JS)) +
  ...
  theme_minimal() + theme(legend.position = "none") +
  annotate("text", x = 1.5, y = max(log1p(F2_2_a$ADR2L_J0), na.rm = TRUE), label = paste("p:", p_value_VTI_2), size = 5) +
  geom_text(data = count_data2,
            aes(x = VTI_JS, y = min(log1p(F2_2_a$ADR2L_J0), na.rm = TRUE) * 1.25, label = paste0("n=", n)),
...
ggsave("FIGURES/Figure2_VTI_Lymphocyte_ADRB.pdf",
       plot   = Figure2_tot,
...
mw_3a <- wilcox.test(log1p(M_ADR1_J0) ~ VTI_JS, data = F3_1)
p_value_VTI <- sprintf("%.3f", mw_3a$p.value)

count_data <- F3_1 %>% group_by(VTI_JS) %>% summarise(n = n(), .groups = "drop")
...
Figure3a <- ggplot(F3_1, aes(x = VTI_JS, y = log1p(M_ADR1_J0), fill = VTI_JS)) +
  ...
  theme_minimal() + theme(legend.position = "none") +
  annotate("text", x = 1.5, y = max(log1p(F3_1$M_ADR1_J0), na.rm = TRUE), label = paste("p:", p_value_VTI), size = 5)
...
mw_3b <- wilcox.test(log1p(M_ADR2_J0) ~ VTI_JS, data = F3_2)
p_value_VTI_2 <- sprintf("%.3f", mw_3b$p.value)

count_data2 <- F3_2 %>% group_by(VTI_JS) %>% summarise(n = n(), .groups = "drop")
...
Figure3b <- ggplot(F3_2, aes(x = VTI_JS, y = log1p(M_ADR2_J0), fill = VTI_JS)) +
  ...
  theme_minimal() + theme(legend.position = "none") +
  annotate("text", x = 1.5, y = max(log1p(F3_2$M_ADR2_J0), na.rm = TRUE), label = paste("p:", p_value_VTI_2), size = 5) +
  geom_text(data = count_data2, aes(x = VTI_JS, y = min(log1p(F3_2$M_ADR2_J0), na.rm = TRUE) * 1.25, label = paste0("n=", n)), inherit.aes = FALSE, size = 4)
...
ggsave("FIGURES/Figure3_VTI_Monocyte_ADRB.pdf",
       plot   = Figure3_tot,
       width  = 200, height = 120, units = "mm",
       device = cairo_pdf, bg = "white")
```

# Figure 4: Association between IL-6 at admission and VTI
...
mw_F4  <- wilcox.test(log1p(IL6_J0) ~ VTI_JS, data = F4_1)
p_F4   <- sprintf("%.3f", mw_F4$p.value)
n_F4   <- F4_1 %>% count(VTI_JS)

Figure4 <- ggplot(F4_1, aes(x = VTI_JS, y = log1p(IL6_J0), fill = VTI_JS)) +
  geom_boxplot() +
  geom_point(size = 2, alpha = 0.7, position = position_jitter(width = 0.1)) +
  scale_fill_manual(values = c("≤12 cm" = "#0072B2", ">12 cm" = "#D55E00")) +
  labs(
  ...
  theme_minimal() + theme(legend.position = "none") +
  annotate("text", x = 1.5, y = max(log1p(F4_1$IL6_J0), na.rm = TRUE),
           label = paste("p:", p_F4), size = 5, color = "black") +
  geom_text(data = n_F4,
            aes(x = VTI_JS, y = min(log1p(F4_1$IL6_J0), na.rm = TRUE) * 1.25, label = paste0("n=", n)),
            inherit.aes = FALSE, size = 4)

Figure4

ggsave("FIGURES/Figure4_VTI_IL6.pdf",
       plot   = Figure4,
...
ggsave("FIGURES/Figure5_IL6.pdf",      fig_IL6,  width=160, height=120, units="mm", device=cairo_pdf, bg="white")
ggsave("FIGURES/Figure5_IL5.pdf",      fig_IL5,  width=160, height=120, units="mm", device=cairo_pdf, bg="white")
ggsave("FIGURES/Figure5_IL8.pdf",      fig_IL8,  width=160, height=120, units="mm", device=cairo_pdf, bg="white")
ggsave("FIGURES/Figure5_IL10.pdf",     fig_IL10, width=160, height=120, units="mm", device=cairo_pdf, bg="white")
ggsave("FIGURES/Figure5_GDF15.pdf",    fig_GDF,  width=160, height=120, units="mm", device=cairo_pdf, bg="white")
```

# Figure 6: Survival ~ IL-6 admission
...
ggsave("FIGURES/Figure6_Survival_IL6_censored.pdf",
       plot   = Figure_6_censored$plot,
       width  = 180, height = 140, units = "mm",
       device = cairo_pdf, bg = "white")
```

# Figure 7: Association between IL-6 and vasopressor dose

```{r}
F7 <- df %>% select(IL6_J0, NADcum_D0)
...
Figure7 <- ggplot(df, aes(x = rank_IL6, y = rank_NAD)) +
...
ggsave("FIGURES/Figure7_IL6_vs_NAD_SpearmanRank.pdf",
       plot   = Figure7,
       width  = 160, height = 120, units = "mm",
       device = cairo_pdf, bg = "white")
```

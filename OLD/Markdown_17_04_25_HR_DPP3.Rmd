...
# HR analysis with ADR in Lymphocyte
...
## No correlation between DPP3 at baseline and ADRB
...
# No correaltion between ADRB at baseline and trajectory of DPP3
---------------------------------------------
---------------------------------------------
...
df_outcome <- df %>%
  filter(Outcome %in% c("ECMO Weaning", "Death")) %>%  
  mutate(Outcome_bin = case_when(
    Outcome == "ECMO Weaning" ~ "Weaning",
    Outcome == "Death" ~ "No Weaning"
  ))
...
# No difference between DPP3 at baseline in weaning/no weaning
...
# No difference between DPP3 at baseline in death/no death

```

# Ggplot and trajectory of DPP3
...
df_assoc <- df %>%
  mutate(
    ...
    Trend_DPP3 = case_when(
      DPP3_D0 > lag(DPP3_D0) ~ "Increase",
      DPP3_D0 < lag(DPP3_D0) ~ "Decrease",
      TRUE ~ "No Change"
    )
  )

df_assoc_clean <- df_assoc %>%
  filter(Trend_DPP3 %in% c("Increase", "Decrease"), !is.na(Outcome_bin), !is.na(Trend_DPP3))

# Ora puoi fare il plot con solo "Increase" e "Decrease"
df_assoc_clean %>% 

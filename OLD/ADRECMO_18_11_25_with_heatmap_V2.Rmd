...
source("FUNCTIONS/FIGURE1 function.R")
source("FUNCTIONS/test_linearité.R")
```
...
Figure2_tot <- (Figure_2_1 + Figure_2_2 + Figure_2_3 + Figure_2_4) +
...
```{r Figure6-display, fig.width=8, fig.height=6, fig.scale=1.5, dpi=300, fig.align='center'}
fig_IL6
fig_IL5
fig_IL8
fig_IL10
...
km_fit_comp <- survfit(Surv(diff_days, outcome_censored) ~ IL6_median , data = F7)
mod_censored <- coxph(Surv(diff_days, outcome_censored) ~ IL6_median , data = F7)
...
Figure_7_censored <- ggsurvplot(km_fit_comp, data = F7, 
...
Figure_7_censored$plot <- Figure_7_censored$plot +
...
make_il6_nad_plot <- function(data, xvar, yvar, title_lab) {
  # test Spearman
  ...
  ggplot(data, aes(x = .data[[xvar]], y = .data[[yvar]])) +
    ...
    labs(
      title = title_lab,
      x = "log(IL-6 + 1)",
      y = "log(Cumulative norepinephrine dose (mg/kg/day)+1)"
    ) +
...
# D0
Figure8_D0 <- make_il6_nad_plot(F8,
                                xvar = "log_value_IL6_D0",
                                yvar = "NADcum_D0_w",
                                title_lab = "A: D0")

# J3–J5
Figure8_J3J5 <- make_il6_nad_plot(F8,
                                  xvar = "log_value_IL6_J3_J5",
                                  yvar = "jp2_NAd_cum_w",
                                  title_lab = "B: D3–D5")

# JS
Figure8_JS <- make_il6_nad_plot(F8,
                                xvar = "log_value_IL6_JS",
                                yvar = "jsev_NAd_cum_w",
                                title_lab = "C: DS")
...
# Panel Figure 8 (3 temps côte à côte)
Figure8 <- Figure8_D0 | Figure8_J3J5 | Figure8_JS
Figure8

# Export en PDF (figure 8 complète)
...
ggplot(F9, aes(x = log_value_IL6_D0, y = j0_ecmo_debit)) +
  geom_point() +
...
# ------------------------------
# 2) Corrélations Spearman
# ------------------------------
cor_D0 <- cor.test(F9$j0_ecmo_debit,   F9$log_value_IL6_D0,    method = "spearman")
cor_J3 <- cor.test(F9$jp2_ecmo_debit,  F9$log_value_IL6_J3_J5, method = "spearman")
cor_JS <- cor.test(F9$jsev_ecmo_debit, F9$log_value_IL6_JS,    method = "spearman")

rho_D0 <- round(cor_D0$estimate, 2); p_D0  <- signif(cor_D0$p.value, 2)
rho_J3 <- round(cor_J3$estimate, 2); p_J3  <- signif(cor_J3$p.value, 2)
rho_JS <- round(cor_JS$estimate, 2); p_JS  <- signif(cor_JS$p.value, 2)

# ------------------------------
# 3) Figures
# ------------------------------

F9_plot <- ggplot(F9, aes(x = log_value_IL6_D0, y = j0_ecmo_debit)) +
  geom_point()+
  geom_smooth(method = "loess", span = 0.9, se = TRUE) +
  annotate("text", x = Inf, y = Inf,
           hjust = 1.1, vjust = 1.5, size = 5,
           label = paste0("Spearman ρ = ", rho_D0, "\np = ", p_D0)) +
  labs(x = "log(IL-6 + 1) D0", y = "ECMO Flow D0 (L/min)",
       title = "A: D0") +
  theme_classic()

F9_plot_2 <- ggplot(F9, aes(x = log_value_IL6_J3_J5, y = jp2_ecmo_debit)) +
  geom_point()+
  geom_smooth(method = "loess", span = 0.9, se = TRUE) +
  annotate("text", x = Inf, y = Inf,
           hjust = 1.1, vjust = 1.5, size = 5,
           label = paste0("Spearman ρ = ", rho_J3, "\np = ", p_J3)) +
  labs(x = "log(IL-6 + 1) D3-D5", y = "ECMO Flow D3–D5 (L/min)",
       title = "B: D3–D5") +
  theme_classic()

F9_plot_3 <- ggplot(F9, aes(x = log_value_IL6_JS, y = jsev_ecmo_debit)) +
  geom_point()+
  geom_smooth(method = "loess", span = 0.9, se = TRUE) +
  annotate("text", x = Inf, y = Inf,
           hjust = 1.1, vjust = 1.5, size = 5,
           label = paste0("Spearman ρ = ", rho_JS, "\np = ", p_JS)) +
  labs(x = "log(IL-6 + 1) weaning day", y = "ECMO Flow weaning day (L/min)",
       title = "C: Weaning day") +
  theme_classic()
```
...
cor_D0 <- cor.test(F10$log_value_IL6_D0,   F10$duree_NAd,    method = "spearman")
rho_D0 <- round(cor_D0$estimate, 2); 
p_D0  <- signif(cor_D0$p.value, 2)


F10_plot <- ggplot(F10, aes(x = log_value_IL6_D0, y = duree_NAd)) +
  geom_point(aes(color = factor(j90_deces)), alpha = 0.8, size = 2) +
  geom_smooth(method = "loess", span = 0.9, se = TRUE) +
  annotate("text", x = Inf, y = Inf,
           hjust = 1.1, vjust = 1.5, size = 5,
           label = paste0("Spearman ρ = ", rho_JS, "\np = ", p_JS)) +
  labs(x = "log(IL-6 + 1) D0", y = "Norepinephrine duration",
       title = "") +
  theme_classic()
```
...
# Sous-donnée IL-6 + SOFA aux 3 temps
F11 <- df %>%
  select(
    IL6_J0, IL6_J3_J5, IL6_JS,
    sofa_D0, jp2_sofa, jsev_sofa
  ) %>%
  mutate(
    log_IL6_D0    = log1p(IL6_J0),
    log_IL6_J3J5  = log1p(IL6_J3_J5),
    log_IL6_JS    = log1p(IL6_JS)
  )
data=F11
il6_var ="log_IL6_D0"
sofa_var = "sofa_D0"
...
F11_D0   <- make_il6_sofa_panel(F11,
                                il6_var   = "log_IL6_D0",
                                sofa_var  = "sofa_D0",
                                panel_label = "A",
                                time_label  = "D0")

F11_J3J5 <- make_il6_sofa_panel(F11,
                                il6_var   = "log_IL6_D0",
                                sofa_var  = "jp2_sofa",
                                panel_label = "B",
                                time_label  = "D3–D5")

F11_JS   <- make_il6_sofa_panel(F11,
                                il6_var   = "log_IL6_D0",
                                sofa_var  = "jsev_sofa",
                                panel_label = "C",
...
# 1) Préparation des données
# ------------------------------
### --- Fonction CKD-EPI Créatinine 2009 --- ###
...
make_il6_ckd_plot <- function(data, xvar, yvar, title_lab) {
  # test Spearman
  ct <- cor.test(data[[xvar]], data[[yvar]],
                 method = "spearman", use = "complete.obs")
  rho  <- round(unname(ct$estimate), 2)
...
# 2) Corrélations Spearman
# ------------------------------
cor_D0 <- cor.test(F13$CKD_D0,   F13$log_value_IL6_D0,    method = "spearman")
cor_J3 <- cor.test(F13$CKD_J3J5,  F13$log_value_IL6_J3_J5, method = "spearman")
cor_JS <- cor.test(F13$CKD_JS, F13$log_value_IL6_JS,    method = "spearman")
...
F13_plot <- ggplot(F13, aes(x = log_value_IL6_D0, y = CKD_D0)) +
  geom_point()+
  geom_smooth(method = "loess", span = 0.9, se = TRUE) +
  annotate("text", x = Inf, y = Inf,
           hjust = 1.1, vjust = 1.5, size = 5,
           label = paste0("Spearman ρ = ", rho_D0, "\np = ", p_D0)) +
  labs(x = "log(IL-6 + 1)", y = "CKD-EPI (ml/min/1.73m²)",
       title = "A: D0") +
  theme_classic()

F13_plot_2 <- ggplot(F13, aes(x = log_value_IL6_J3_J5, y = CKD_J3J5)) +
  geom_point()+
  geom_smooth(method = "loess", span = 0.9, se = TRUE) +
  annotate("text", x = Inf, y = Inf,
           hjust = 1.1, vjust = 1.5, size = 5,
...
F13_plot_3 <- ggplot(F13, aes(x = log_value_IL6_JS, y = CKD_JS)) +
  geom_point()+
  geom_smooth(method = "loess", span = 0.9, se = TRUE) +
  annotate("text", x = Inf, y = Inf,
           hjust = 1.1, vjust = 1.5, size = 5,

...
Figure_7_censored <- ggsurvplot(km_fit_comp, data = df, 
...
F9 <- df%>%
  mutate(preecmo_ett_ann_aor_d_corr = case_when(is.na(preecmo_ett_ann_aor_d) ~ 2, T~preecmo_ett_ann_aor_d),
  ...
  mutate(log_value_IL6_D0 = log1p(IL6_J0),
...
rho_J3 <- round(cor_J3$estimate, 2)
p_J3   <- signif(cor_J3$p.value, 2)

rho_JS <- round(cor_JS$estimate, 2)
p_JS   <- signif(cor_JS$p.value, 2)

# ---- Figures ----

# D0
F9_plot <- F9 %>%
  ggplot(aes(x = log_value_IL6_D0, y = j0_ecmo_debit)) +
  geom_point() +
  ...
  labs(
    x = "log(IL-6 + 1)",
    y = "ECMO Flow D0 (L/min)",
    title = "ECMO Flow D0 vs log-transformed IL-6"
  ) +
  theme_classic()

# J3–J5
F9_plot_2 <- F9 %>%
  ggplot(aes(x = log_value_IL6_J3_J5, y = jp2_ecmo_debit)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE) +
  annotate("text",
           x = Inf, y = Inf, 
  ...
  labs(
    x = "log(IL-6 + 1)",
    y = "ECMO Flow J3–J5 (L/min)",
    title = "ECMO Flow J3–J5 vs log-transformed IL-6"
  ) +
  theme_classic()

# JS
F9_plot_3 <- F9 %>%
  ggplot(aes(x = log_value_IL6_JS, y = jsev_ecmo_debit)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE) +
  annotate("text",
           x = Inf, y = Inf, 
  ...
  labs(
    x = "log(IL-6 + 1)",
    y = "ECMO Flow JS (L/min)",
    title = "ECMO Flow JS vs log-transformed IL-6"
  ) +
  theme_classic()
library(patchwork)

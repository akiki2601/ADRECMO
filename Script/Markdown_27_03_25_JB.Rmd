...
This study is a monocentric, prospective, observational investigation conducted at XX Hospital between [Date] and [Date], enrolling 50 adult patients with refractory cardiogenic shock requiring veno-arterial extracorporeal membrane oxygenation (VA-ECMO) support (NCT03327493).
...
Eligible patients were adults (≥18 years old) affiliated with a national health insurance system. Patients were excluded if cardiogenic shock was secondary to cardiotoxic poisoning, if they were pregnant, had a known active HIV or HCV infection, or were under legal protective supervision (e.g., court-appointed guardianship).
...
var.cat <- c("Gender", "HTN", "DM", "Immunodepression", "CKD", "Neurologic_deficit", "Chronic_respiratory_disease", "HF", "Cardiac_arrest_before_canul", "IABP_D0", "Intubation", "EER_D0", "Betablocker", "Alphablocker", "Levosimendan")

var.cont <- c("Age", "BMI", "PAS_D0", "PAD_D0", "PAM_D0", "Pulsepressure_D0", "HR_D0", "Temp_D0", "Preecmo_tte_ef", "Preecmo_tte_vtiao", "Preecmo_tte_tapse", "Preecmo_tte_vmax_s_mit_lat_LV", "NADcum_D0", "DOBUcum_D0", "pH_D0", "Sao2_D0", "Paco2_D0", "Lact_D0", "Hco3_D0", "Creat_D0", "Urea_D0", "ALAT_D0", "ASAT_D0", "Bili_D0", "PT_D0", "Tropo_i_hs_D0", "Ntprobnp_D0", "Plq_D0", "Lenght_vm", "Time_hosp", "Time_ecmo")
...
table2a <- df_long %>%
  ...
  mutate(`Median [IQR]` = paste0(Median, " [", Q1, "–", Q3, "]")) %>%
  select(Parameter, n, `Median [IQR]`) %>%
...
# Table 2b with hemodynamic parameters at weaning day
...
df_long <- df %>%
  ...
  rename(
    ...
    `Noradrenalin (mg/h)` = jsev_epr_sev_t0_3l_NAd,
    `VTI_1.5L (cm)` = `jsev_epr_sev_t1_1,5l_itv`,
    `MAP_1.5L (mmHg)` = `jsev_epr_sev_t1_1,5l_pam`,
    `Noradrenalin_1.5L (mg/h)` = `jsev_epr_sev_t1_1,5l_NAd`
  ) %>%
  pivot_longer(
    cols = -ID,
...
# Create summary table with n
table2b_compact <- df_long %>%
  ...
  mutate(Summary = paste0("n = ", n, " | ", Median, " [", Q1, "–", Q3, "]")) %>%
  select(Parameter, Condition, Summary) %>%
  pivot_wider(names_from = Condition, values_from = Summary)

# Visualizza tabella
print(table2b_compact)
...
Figure_1 <- ggplot(F1, aes(x = ADR1_time, y = value, fill = Outcomes)) +
...
Figure_1.1 <- ggplot(F1_1, aes(x = ADR2_time, y = value, fill = Outcomes)) +
  geom_boxplot(position = position_dodge(0.7), width = 0.6) +  
  scale_y_log10() +  
  labs(
...
F2_1 <- df %>%
  ...
  dplyr::select(VTI_J3_J5_r, VTI_J3_J5_r_cut, ADR1_J0, ADR1_J3_J5, ADR1_JS, ADR1_J3_J5_r)
...
Figure2_tot <- Fig_VTI_ADRB1 | Fig_VTI_ADRB2 + 
  plot_annotation(title = "Association between VTI at weaning and ADRB1/ADRB2 expression")
...
mann_whitney_result <- wilcox.test(ADR1_J3_J5_r ~ dobu_D3_D5_1_yes_no, data = df4, exact = FALSE)

p_value <- sprintf("%.4g", mann_whitney_result$p.value)

#boxplot ADRB1
Fig_4a <- ggplot(df4, aes(x= dobu_D3_D5_1_yes_no, y = ADR1_J3_J5_r, fill = factor(dobu_D3_D5_1_yes_no))) +
  geom_boxplot() +
  geom_point(size = 2, alpha = 0.7) +
  #scale_fill_manual(values = c("white", "gray70")) +
  ...
  theme_minimal() +
  theme(legend.position = "none")+
  annotate("text", x = 1.5, y = max(df4$ADR1_J3_J5, na.rm = TRUE), 
...
mann_whitney_result <- wilcox.test(ADR2_J3_J5_r ~ dobu_D3_D5_1_yes_no, data = df4, exact = FALSE)

p_value <- sprintf("%.4g", mann_whitney_result$p.value)

Fig_4b <- ggplot(df4, aes(x= dobu_D3_D5_1_yes_no, y = ADR2_J3_J5_r, fill = factor(dobu_D3_D5_1_yes_no))) +
  geom_boxplot() +
  geom_point(size = 2, alpha = 0.7) +
  #scale_fill_manual(values = c("white", "gray70")) +
  ...
  theme_minimal() +
  theme(legend.position = "none")+
  annotate("text", x = 1.5, y = max(df4$ADR2_J3_J5, na.rm = TRUE), 
           label = paste("Mann-Whitney p-value:", p_value), 
           size = 5, color = "black", hjust = 0.5)
...
mann_whitney_NAd_ADRB1 <- wilcox.test(ADR1_J3_J5_r ~ nora_D3_D5_1_yes_no, data = df5, exact = FALSE)

# Extract and format the p-value
p_value_NAd_ADRB1 <- sprintf("%.4g", mann_whitney_NAd_ADRB1$p.value)

# Boxplot for ADRB1 with p-value annotation
Figure_6a <- ggplot(df5, aes(x = nora_D3_D5_1_yes_no, y = ADR1_J3_J5_r, fill = factor(nora_D3_D5_1_yes_no))) +
  geom_boxplot() +
  geom_point(size = 2, alpha = 0.7) +
  labs(
  ...
  theme_minimal() +
  theme(legend.position = "none") +
  annotate("text", x = 1.5, y = max(df5$ADR1_J3_J5_r, na.rm = TRUE), 
           label = paste("Mann-Whitney p-value:", p_value_NAd_ADRB1), 
           size = 5, color = "black", hjust = 0.5)

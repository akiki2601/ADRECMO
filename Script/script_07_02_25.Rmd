...
# N for variable
var_counts <- sapply(var.tot, function(x) sum(!is.na(T1[[x]])))

# names compatible
names(var_counts) <- var.tot

# Modifz rows name for having n
rownames(table1_strata_df) <- paste0(rownames(table1_strata_df), " (n=", var_counts[rownames(table1_strata_df)], ")")

# low first colomn
T1$ADRtot_D0_Strata <- fct_relevel(T1$ADRtot_D0_Strata, "ADRtot% low", "ADRtot% high")
...
## other way for having n (it doesn't work)
...
names(var_counts) <- missing_summary$variable  # for corresponding names

# Standardisation names
rownames(table1_strata_df) <- make.names(rownames(table1_strata_df))  
names(var_counts) <- make.names(names(var_counts))  

# add n
rownames(table1_strata_df) <- ifelse(
  rownames(table1_strata_df) %in% names(var_counts),
  paste0(rownames(table1_strata_df), " (n=", var_counts[rownames(table1_strata_df)], ")"),
  rownames(table1_strata_df)
)

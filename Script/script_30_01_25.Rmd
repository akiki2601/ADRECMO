...
### Table1 with strata ~ SOFA

# Tercile
tercile <- quantile(T1$sofa_D0, probs = c(0, 1/3, 2/3, 1), na.rm = TRUE)
print(tercile)

# Quartile
quartile <- quantile(T1$sofa_D0, probs = c(0, 0.25, 0.5, 0.75, 1), na.rm = TRUE)
print(quartile)

##median is SOFA 9 ->moderate organ dysfunction
...
sofa_median <- median(T1$sofa_D0, na.rm = TRUE)

T1$SOFA_Strata <- ifelse(T1$sofa_D0 >= sofa_median, "high", "low")
...
colnames(table1_strata_df)[2:ncol(table1_strata_df)] <- paste("SOFA =", colnames(table1_strata_df)[2:ncol(table1_strata_df)])
...
table1_flex <- flextable(table1_strata_df) %>%
  ...
  set_caption("Table 1: Population at baseline according to SOFA score")

doc <- read_docx() %>%
  body_add_flextable(table1_flex)

print(doc, target = "TABLES/Table1_SOFA_Strata.docx")

browseURL("TABLES/Table1_SOFA_Strata.docx")

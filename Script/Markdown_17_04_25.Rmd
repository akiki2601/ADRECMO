...
qqnorm(ranef(mod1_m)$ID[, "time"], main = "QQ plot of random slopes (time)")
qqline(ranef(mod1_m)$ID[, "time"], col = "blue")

# ===========================
# Figure
# ===========================
Figure_1m <- ggplot(F1m, aes(x = ADR1_time, y = value_log, fill = Outcomes)) +
  geom_boxplot(position = position_dodge(0.7), width = 0.6) +
  scale_y_log10() +
  labs(
  ...
  geom_text(data = count_data_m, aes(
    x = ADR1_time,
    y = -min(F1m$value_log, na.rm = TRUE) * 0.1,
    label = paste0("n=", n),
    group = Outcomes
  ),
  ...
  annotate(
    "text",
    x = 1,
    y = max(F1m$value_log, na.rm = TRUE) * 1.1,
    label = "p[time] == 0.024*';'~p[outcomes] == 0.066",
    parse = TRUE,
    size = 4,
    hjust = 0
...
quantili_dpp3_j0 <- df %>%
  summarise(
    Q1 = quantile(DPP3_D0, 0.25, na.rm = TRUE),
    Median = median(DPP3_D0, na.rm = TRUE),
    Q3 = quantile(DPP3_D0, 0.75, na.rm = TRUE)
  )

quantili_dpp3_j0

## All value above normal value of DPP3 (30 ng/mL)

----------------------------------
---------------------------------

## Boxplot DPP3 high/low and ADRB1

#  Median DPP3_D0
dpp3_d0_median <- median(df$DPP3_D0, na.rm = TRUE)

# Variable </> median DPP3
df <- df %>%
  mutate(DPP3_D0_group = ifelse(DPP3_D0 > dpp3_d0_median,
                                "High DPP3 (> median)", "Low DPP3 (< median)"))


# p-value Mann-Whitney
p_value_DPP3 <- wilcox.test(ADR1_D0 ~ DPP3_D0_group, data = df)$p.value
p_value_DPP3 <- signif(p_value_DPP3, 3)

# Boxplot
Figure_DPP3 <- ggplot(df, aes(x = DPP3_D0_group, y = ADR1_D0, fill = DPP3_D0_group)) +
  geom_boxplot() +
  geom_point(size = 2, alpha = 0.7, position = position_jitter(width = 0.1)) +
  scale_fill_manual(values = c("Low DPP3 (< median)" = "#E69F00", "High DPP3 (> median)" = "#56B4E9")) +  # Color-blind friendly
  labs(
    title = "Association between DPP3 level at D0 and ADRB1",
    x = "DPP3 at D0 based on median split",
    y = "ADRB1 at D0"
  ) +
  theme_minimal() +
  theme(legend.position = "none") +
  annotate("text", x = 1.5, y = max(df$ADR1_D0, na.rm = TRUE) * 1.1, 
           label = paste("Mann-Whitney p-value:", p_value_DPP3), 
           size = 5, color = "black", hjust = 0.5)
...
## Boxplot DPP3 high/low and ADRB2  
  
# Compute p-value (Mann-Whitney test) for ADR2_D0
p_value_DPP3_ADR2 <- wilcox.test(ADR2_D0 ~ DPP3_D0_group, data = df, exact = FALSE)$p.value
p_value_DPP3_ADR2 <- signif(p_value_DPP3_ADR2, 3)

# Build the figure
Figure_DPP3_ADR2 <- ggplot(df, aes(x = DPP3_D0_group, y = ADR2_D0, fill = DPP3_D0_group)) +
  geom_boxplot() +
  geom_point(size = 2, alpha = 0.7, position = position_jitter(width = 0.1)) +
  scale_fill_manual(values = c("Low DPP3 (< median)" = "#E69F00", 
                               "High DPP3 (> median)" = "#56B4E9")) +  # Color-blind friendly
  labs(
    title = "Association between DPP3 level at D0 and ADRB2",
    x = "DPP3 at D0 based on median split",
    y = "ADRB2 at D0"
  ) +
  theme_minimal() +
  theme(legend.position = "none") +
  annotate("text", x = 1.5, y = max(df$ADR2_D0, na.rm = TRUE) * 1.1, 
           label = paste("Mann-Whitney p-value:", p_value_DPP3_ADR2), 
           size = 5, color = "black", hjust = 0.5)

# Print the figure
print(Figure_DPP3_ADR2)


# Print the figure
print(Figure_DPP3_ADRB2)

---------------------------------------------
---------------------------------------------
## Boxplot DPP3 decrease/increase at D3 and ADRB1  

df <- df %>%
  mutate(Trend_DPP3 = case_when(
    is.na(DPP3_D0) | is.na(DPP3_D3_5) ~ NA_character_,
    DPP3_D3_5 > DPP3_D0 ~ "Increase",
    DPP3_D3_5 < DPP3_D0 ~ "Decrease",
    TRUE ~ NA_character_
  ))

df_plot <- df %>%
  filter(Trend_DPP3 %in% c("Increase", "Decrease"))

p_value_traj <- wilcox.test(ADR1_D0 ~ Trend_DPP3, data = df_plot, exact = FALSE)$p.value
p_value_traj <- signif(p_value_traj, 3)

Figure_ADR1_Trajectory <- ggplot(df_plot, aes(x = Trend_DPP3, y = ADR1_D0, fill = Trend_DPP3)) +
  geom_boxplot() +
  geom_point(size = 2, alpha = 0.7, position = position_jitter(width = 0.1)) +
  scale_fill_manual(values = c("Decrease" = "#E69F00", "Increase" = "#56B4E9")) +
  labs(
    title = "Association between DPP3 trajectory and ADRB1",
    x = "DPP3 trajectory (D0 → D3-5)",
    y = "ADRB1 at D0"
  ) +
  theme_minimal() +
  theme(legend.position = "none") +
  annotate("text", x = 1.5, y = max(df_plot$ADR1_D0, na.rm = TRUE) * 1.1,
           label = paste("Mann-Whitney p-value:", p_value_traj),
           size = 5, color = "black", hjust = 0.5)

print(Figure_ADR1_Trajectory)


## Boxplot DPP3 decrease/increase at D3 and ADRB2

df_plot_adr2 <- df %>%
  filter(Trend_DPP3 %in% c("Increase", "Decrease"))

p_value_traj_adr2 <- wilcox.test(ADR2_D0 ~ Trend_DPP3, data = df_plot_adr2, exact = FALSE)$p.value
p_value_traj_adr2 <- signif(p_value_traj_adr2, 3)

Figure_ADR2_Trajectory <- ggplot(df_plot_adr2, aes(x = Trend_DPP3, y = ADR2_D0, fill = Trend_DPP3)) +
  geom_boxplot() +
  geom_point(size = 2, alpha = 0.7, position = position_jitter(width = 0.1)) +
  scale_fill_manual(values = c("Decrease" = "#E69F00", "Increase" = "#56B4E9")) +
  labs(
    title = "Association between DPP3 trajectory and ADRB2",
    x = "DPP3 trajectory (D0 → D3-5)",
    y = "ADRB2 at D0"
  ) +
  theme_minimal() +
  theme(legend.position = "none") +
  annotate("text", x = 1.5, y = max(df_plot_adr2$ADR2_D0, na.rm = TRUE) * 1.1,
           label = paste("Mann-Whitney p-value:", p_value_traj_adr2),
           size = 5, color = "black", hjust = 0.5)

# Stampa il grafico
print(Figure_ADR2_Trajectory)

---------------------------------------------
---------------------------------------------
## Boxplot DPP3 decrease/increase at Dw and ADRB1 


df <- df %>%
  mutate(Trend_DPP3_weaning = case_when(
    is.na(DPP3_D0) | is.na(DPP3_w) ~ NA_character_,
    DPP3_w > DPP3_D0 ~ "Increase",
    DPP3_w < DPP3_D0 ~ "Decrease",
    TRUE ~ NA_character_
  ))

df_plot_adrb1 <- df %>%
  filter(Trend_DPP3_weaning %in% c("Increase", "Decrease"))

p_adrb1_traj_w <- wilcox.test(ADR1_D0 ~ Trend_DPP3_weaning, data = df_plot_adrb1, exact = FALSE)$p.value
p_adrb1_traj_w <- signif(p_adrb1_traj_w, 3)

# Boxplot
Figure_ADRB1_w <- ggplot(df_plot_adrb1, aes(x = Trend_DPP3_weaning, y = ADR1_D0, fill = Trend_DPP3_weaning)) +
  geom_boxplot() +
  geom_point(size = 2, alpha = 0.7, position = position_jitter(width = 0.1)) +
  scale_fill_manual(values = c("Decrease" = "#E69F00", "Increase" = "#56B4E9")) +
  labs(
    title = "ADRB1 at D0 by DPP3 trajectory (D0 → Weaning)",
    x = "DPP3 trajectory",
    y = "ADRB1 at D0"
  ) +
  theme_minimal() +
  theme(legend.position = "none") +
  annotate("text", x = 1.5, y = max(df_plot_adrb1$ADR1_D0, na.rm = TRUE) * 1.1,
           label = paste("Mann-Whitney p-value:", p_adrb1_traj_w),
           size = 5, color = "black", hjust = 0.5)

print(Figure_ADRB1_w)
...
# Calcola p-value
p_adrb2_traj_w <- wilcox.test(ADR2_D0 ~ Trend_DPP3_weaning, data = df_plot_adrb2, exact = FALSE)$p.value
p_adrb2_traj_w <- signif(p_adrb2_traj_w, 3)

# Boxplot
Figure_ADRB2_w <- ggplot(df_plot_adrb2, aes(x = Trend_DPP3_weaning, y = ADR2_D0, fill = Trend_DPP3_weaning)) +
  geom_boxplot() +
  geom_point(size = 2, alpha = 0.7, position = position_jitter(width = 0.1)) +
  scale_fill_manual(values = c("Decrease" = "#E69F00", "Increase" = "#56B4E9")) +
  labs(
    title = "ADRB2 at D0 by DPP3 trajectory (D0 → Weaning)",
    x = "DPP3 trajectory",
    y = "ADRB2 at D0"
  ) +
  theme_minimal() +
  theme(legend.position = "none") +
  annotate("text", x = 1.5, y = max(df_plot_adrb2$ADR2_D0, na.rm = TRUE) * 1.1,
           label = paste("Mann-Whitney p-value:", p_adrb2_traj_w),
           size = 5, color = "black", hjust = 0.5)

print(Figure_ADRB2_w)

# here only one were ADRB1/2 low had congreunt trend with DPP3 trajectory =increase. P never significant.
  

-------------------------------------------
--------------------------------------------
## Boxplot DPP3 and ECMO weaning

# Create binary outcome: Weaning vs No Weaning
df_outcome <- df %>%
  filter(Outcome %in% c(1, 4)) %>%
  mutate(Outcome_bin = case_when(
    Outcome == 1 ~ "Weaning",
    Outcome == 4 ~ "No Weaning"
  ))

# p-value
p_dpp3_d0 <- wilcox.test(DPP3_D0 ~ Outcome_bin, data = df_outcome, exact = FALSE)$p.value
p_dpp3_d0 <- signif(p_dpp3_d0, 3)

# Boxplot
Figure_DPP3_D0 <- ggplot(df_outcome, aes(x = Outcome_bin, y = DPP3_D0, fill = Outcome_bin)) +
  geom_boxplot() +
  geom_point(position = position_jitter(width = 0.1), size = 2, alpha = 0.7) +
  scale_fill_manual(values = c("Weaning" = "#56B4E9", "No Weaning" = "#E69F00")) +
  labs(
    title = "DPP3 at D0 by ECMO Weaning Outcome",
    x = "Outcome",
    y = "DPP3 at D0"
  ) +
  theme_minimal() +
  theme(legend.position = "none") +
  annotate("text", x = 1.5, y = max(df_outcome$DPP3_D0, na.rm = TRUE) * 1.1, 
           label = paste("Mann-Whitney p-value:", p_dpp3_d0),
           size = 5, color = "black", hjust = 0.5)

print(Figure_DPP3_D0)

---------------------------------------------
---------------------------------------------
## Boxplot DPP3 and death

# Create binary outcome: Death vs No Death
df_outcome_death <- df %>%
  mutate(Outcome_bin = case_when(
    Outcome == 4 ~ "Death",
    TRUE ~ "No Death"
...
# p-value
p_dpp3_d0_death <- wilcox.test(DPP3_D0 ~ Outcome_bin, data = df_outcome_death, exact = FALSE)$p.value
p_dpp3_d0_death <- signif(p_dpp3_d0_death, 3)

# Boxplot
Figure_DPP3_Death <- ggplot(df_outcome_death, aes(x = Outcome_bin, y = DPP3_D0, fill = Outcome_bin)) +
  geom_boxplot() +
  geom_point(position = position_jitter(width = 0.1), size = 2, alpha = 0.7) +
  scale_fill_manual(values = c("No Death" = "#56B4E9", "Death" = "#E69F00")) +
  labs(
    title = "DPP3 at D0 by Mortality Outcome",
    x = "Outcome",
    y = "DPP3 at D0"
  ) +
  theme_minimal() +
  theme(legend.position = "none") +
  annotate("text", x = 1.5, y = max(df_outcome_death$DPP3_D0, na.rm = TRUE) * 1.1, 
           label = paste("Mann-Whitney p-value:", p_dpp3_d0_death),
           size = 5, color = "black", hjust = 0.5)

print(Figure_DPP3_Death)

----------------------------------------
----------------------------------------
## Ggplot DPP3 trajectory at D3 and weaning  
  
# Filtra solo pazienti con Outcome 1 (Weaning) o 4 (Death) e DPP3 trend valido
df_assoc <- df %>%
  filter(Outcome %in% c(1, 4), Trend_DPP3 %in% c("Increase", "Decrease")) %>%
  mutate(
...
table_assoc <- table(df_assoc$Trend_DPP3, df_assoc$Outcome_bin)
print(table_assoc)


df_assoc %>%
  count(Trend_DPP3, Outcome_bin) %>%
  group_by(Trend_DPP3) %>%
  mutate(percent = round(100 * n / sum(n), 1)) %>%
  ggplot(aes(x = Trend_DPP3, y = percent, fill = Outcome_bin)) +
  geom_col(position = "stack") +
  scale_fill_manual(values = c("Weaning" = "#56B4E9", "No Weaning" = "#E69F00")) +
  labs(
    title = "Weaning vs No Weaning by DPP3 trajectory",
    x = "DPP3 trajectory (D0 to D3-5)",
    y = "Percentage of patients"
  ) +
  theme_minimal()


fisher_result_wi <- fisher.test(table_assoc)
fisher_result_wi
...
## Ggplot DPP3 trajectory at Dweaning and weaning 

#Filter
df_assoc_w <- df %>%
  filter(Outcome %in% c(1, 4), Trend_DPP3_weaning %in% c("Increase", "Decrease")) %>%
  mutate(
    Outcome_bin = case_when(
      Outcome == 1 ~ "Weaning",
      Outcome == 4 ~ "No Weaning"
...
table_weaning_w <- table(df_assoc_w$Trend_DPP3_weaning, df_assoc_w$Outcome_bin)
print(table_weaning_w)

# Test di Fisher
fisher_result_w <- fisher.test(table_weaning_w)
fisher_result_w

#Table
df_assoc_w %>%
  count(Trend_DPP3_weaning, Outcome_bin) %>%
  group_by(Trend_DPP3_weaning) %>%
  mutate(percent = round(100 * n / sum(n), 1)) %>%
  ggplot(aes(x = Trend_DPP3_weaning, y = percent, fill = Outcome_bin)) +
  geom_col(position = "stack") +
  scale_fill_manual(values = c("Weaning" = "#56B4E9", "No Weaning" = "#E69F00")) +
  labs(
    title = "Weaning vs No Weaning by DPP3 trajectory (D0 to Weaning)",
    x = "DPP3 trajectory (D0 → Weaning)",
    y = "Percentage of patients"
  ) +
  theme_minimal()


## Ggplot DPP3 trajectory at D3 and death

#Filter
df_assoc_death <- df %>%
  filter(Outcome %in% c(1, 2, 3, 4), Trend_DPP3 %in% c("Increase", "Decrease")) %>%
  mutate(
    Outcome_bin = case_when(
      Outcome == 4 ~ "Death",
      TRUE ~ "No Death"
...
#Table
table_dpp3_death <- table(df_assoc_death$Trend_DPP3, df_assoc_death$Outcome_bin)
print(table_dpp3_death)

# Fisher exact test
fisher_dpp3_death <- fisher.test(table_dpp3_death)
fisher_dpp3_death
...
df_assoc_death %>%
  ...
  scale_fill_manual(values = c("No Death" = "#56B4E9", "Death" = "#E69F00")) +
  labs(
    title = "Death vs No Death by DPP3 trajectory",
    x = "DPP3 trajectory (D0 to D3-5)",
    y = "Percentage of patients"
  ) +
  theme_minimal()

## Ggplot DPP3 trajectory at Dweaning and death

# Filter
df_assoc_death_w <- df %>%
  filter(Outcome %in% c(1, 2, 3, 4), Trend_DPP3_weaning %in% c("Increase", "Decrease")) %>%
  mutate(
    Outcome_bin = case_when(
      Outcome == 4 ~ "Death",
      TRUE ~ "No Death"
...
# Table
table_dpp3w_death <- table(df_assoc_death_w$Trend_DPP3_weaning, df_assoc_death_w$Outcome_bin)
print(table_dpp3w_death)

# Test Fisher
fisher_dpp3w_death <- fisher.test(table_dpp3w_death)
fisher_dpp3w_death
...
df_assoc_death_w %>%
  ...
  scale_fill_manual(values = c("No Death" = "#56B4E9", "Death" = "#E69F00")) +
  labs(
    title = "Death vs No Death by DPP3 trajectory (D0 → Weaning)",
    x = "DPP3 trajectory",
    y = "Percentage of patients"
  ) +
  theme_minimal()
...
F_dpp3 <- df %>%
  select(ID, DPP3_D0, DPP3_D3_5, DPP3_w, Outcome) %>%
  mutate(
    Outcome_txt = case_when(
      Outcome == 1 ~ "ECMO Weaning",
      Outcome == 2 ~ "Bridge to Transplant",
      Outcome == 3 ~ "Bridge to LVAD",
      Outcome == 4 ~ "Death"
    ...
    Outcomes = factor(
      case_when(
        Outcome_txt %in% c("Bridge to Transplant", "Bridge to LVAD") ~ "Bridge to transplant or LVAD",
        TRUE ~ Outcome_txt
      ),
      levels = c("ECMO Weaning", "Bridge to transplant or LVAD", "Death")
    )
  ...
  pivot_longer(
    cols = c(DPP3_D0, DPP3_D3_5, DPP3_w),
    names_to = "DPP3_time",
    values_to = "value"
  ) %>%
  mutate(
    time = as.numeric(case_when(
      DPP3_time == "DPP3_D0" ~ 1,
      DPP3_time == "DPP3_D3_5" ~ 2,
      DPP3_time == "DPP3_w" ~ 3
    )),
    value_log = log10(value)
  )

count_data_dpp3 <- F_dpp3 %>%
  filter(!is.na(value)) %>%
  group_by(DPP3_time, Outcomes) %>%
  summarise(n = n(), .groups = "drop") %>%
  filter(!is.na(Outcomes))

# Modello con interazione
mod_dpp3 <- lmerTest::lmer(
  value_log ~ time * Outcomes + (1 + time | ID),
  data = F_dpp3
)

# Modello ridotto
mod_dpp3_red <- lmerTest::lmer(
  value_log ~ time + Outcomes + (1 + time | ID),
  data = F_dpp3
)

# Test interazione
anova(mod_dpp3_red, mod_dpp3)

# Test effetti fissi
anova(mod_dpp3_red, ddf = "Kenward-Roger")

plot(mod_dpp3)
plot(fitted(mod_dpp3), residuals(mod_dpp3),
     xlab = "Fitted values", ylab = "Residuals",
     main = "Residuals vs Fitted")
qqnorm(residuals(mod_dpp3)); qqline(residuals(mod_dpp3), col = "blue")
acf(residuals(mod_dpp3), main = "ACF of residuals")
qqnorm(ranef(mod_dpp3)$ID[, "time"]); qqline(ranef(mod_dpp3)$ID[, "time"], col = "blue")

Figure_DPP3 <- ggplot(F_dpp3, aes(x = DPP3_time, y = value_log, fill = Outcomes)) +
  geom_boxplot(position = position_dodge(0.7), width = 0.6) +
  scale_y_log10() +
  labs(
    title = "Association between DPP3 and outcomes",
    x = "Timepoints of measurement",
    y = "Plasma DPP3 (log10 scale)",
    fill = "Outcome"  # ← nome della legenda
  ) +
  geom_text(data = count_data_dpp3, aes(x = DPP3_time, y = -min(F_dpp3$value_log, na.rm = TRUE) * 0.1,
                                        label = paste0("n=", n), group = Outcomes),
            position = position_dodge(0.7), size = 4, vjust = 1) +
  scale_x_discrete(labels = c("DPP3_D0" = "implantation", "DPP3_D3_5" = "day 3 to 5", "DPP3_w" = "explantation")) +
  theme_minimal() +
  theme(legend.position = "right") +
  annotate(
    "text",
    x = 1,
    y = max(F_dpp3$value_log, na.rm = TRUE) * 1.1,
    label = "p[time] == 0.001*';'~p[outcomes] == 0.83",
    parse = TRUE,
    size = 4,
    hjust = 0
  )
...
-----------------------------------------------
-----------------------------------------------
## Correlation DPP3 - ADRB
  
F3c_dpp3_adrb1 <- df %>%
  dplyr::select(DPP3_D0, DPP3_D3_5, DPP3_w, ADR1_D0, ADR1_J3_J5, ADR1_JS) %>%
  mutate(
    # Correlazioni separate per i timepoint
    correlation_D0 = sprintf("%.4f", cor(DPP3_D0, ADR1_D0, method = "spearman", use = "complete.obs")),
    correlation_D3_5 = sprintf("%.4f", cor(DPP3_D3_5, ADR1_J3_J5, method = "spearman", use = "complete.obs")),
    correlation_w = sprintf("%.4f", cor(DPP3_w, ADR1_JS, method = "spearman", use = "complete.obs"))
  )

# Scatterplot con regressione e annotazione per ogni timepoint
Fig_DPP3_ADRB1_D0 <- ggplot(F3c_dpp3_adrb1, aes(x = DPP3_D0, y = ADR1_D0)) +
  geom_point(color = "blue", alpha = 0.6) +
  geom_smooth(method = "lm", color = "red", se = TRUE) +
  labs(
    title = "Correlation between DPP3 at D0 and ADRB1 at D0",
    x = "DPP3 at D0",
    y = "ADRB1 at D0"
  ) +
  stat_cor(method = "spearman", label.x = min(F3c_dpp3_adrb1$DPP3_D0, na.rm = TRUE),
           label.y = max(F3c_dpp3_adrb1$ADR1_D0, na.rm = TRUE),
           size = 5, color = "black") +
  theme_minimal()


Fig_DPP3_ADRB1_D3_5 <- ggplot(df, aes(x = DPP3_D3_5, y = ADR1_J3_J5)) +
  geom_point(color = "green", alpha = 0.6) +
  geom_smooth(method = "lm", color = "red", se = TRUE) +
  stat_cor(method = "spearman", label.x = min(df$DPP3_D3_5, na.rm = TRUE), 
           label.y = max(df$ADR1_J3_J5, na.rm = TRUE), size = 5) +
  labs(
    title = "Correlation between DPP3 and ADRB1 at D3–5",
    x = "DPP3 at D3–5",
    y = "ADRB1 at D3–5"
  ) +
  theme_minimal()
...
Fig_DPP3_ADRB1_w <- ggplot(df, aes(x = DPP3_w, y = ADR1_JS)) +
  geom_point(color = "red", alpha = 0.6) +
  geom_smooth(method = "lm", color = "red", se = TRUE) +
  stat_cor(method = "spearman", label.x = min(df$DPP3_w, na.rm = TRUE), 
           label.y = max(df$ADR1_JS, na.rm = TRUE), size = 5) +
  labs(
    title = "Correlation between DPP3 and ADRB1 at Weaning",
    x = "DPP3 at Weaning",
    y = "ADRB1 at Weaning"
  ) +
  theme_minimal()
...
print(Fig_DPP3_ADRB1_D0)
print(Fig_DPP3_ADRB1_D3_5)
print(Fig_DPP3_ADRB1_w)


----------------------------------
----------------------------------
  
# Seleziona variabili per DPP3 e ADRB2 ai tre timepoint
F3c_dpp3_adrb2 <- df %>%
  dplyr::select(DPP3_D0, DPP3_D3_5, DPP3_w, ADR2_D0, ADR2_J3_J5, ADR2_JS) %>%
  mutate(
    correlation_D0 = sprintf("%.4f", cor(DPP3_D0, ADR2_D0, method = "spearman", use = "complete.obs")),
    correlation_D3_5 = sprintf("%.4f", cor(DPP3_D3_5, ADR2_J3_J5, method = "spearman", use = "complete.obs")),
    correlation_w = sprintf("%.4f", cor(DPP3_w, ADR2_JS, method = "spearman", use = "complete.obs"))
  )

Fig_DPP3_ADRB2_D0 <- ggplot(F3c_dpp3_adrb2, aes(x = DPP3_D0, y = ADR2_D0)) +
  geom_point(color = "blue", alpha = 0.6) +
  geom_smooth(method = "lm", color = "red", se = TRUE) +
  labs(
    title = "Correlation between DPP3 at D0 and ADRB2 at D0",
    x = "DPP3 at D0",
    y = "ADRB2 at D0"
  ) +
  annotate("text", x = min(F3c_dpp3_adrb2$DPP3_D0, na.rm = TRUE),
           y = max(F3c_dpp3_adrb2$ADR2_D0, na.rm = TRUE),
           label = paste("Spearman correlation:", F3c_dpp3_adrb2$correlation_D0[1]),
           hjust = 0, size = 5, color = "black") +
  theme_minimal()

Fig_DPP3_ADRB2_D3_5 <- ggplot(F3c_dpp3_adrb2, aes(x = DPP3_D3_5, y = ADR2_J3_J5)) +
  geom_point(color = "green", alpha = 0.6) +
  geom_smooth(method = "lm", color = "red", se = TRUE) +
  labs(
    title = "Correlation between DPP3 at D3–5 and ADRB2 at D3–5",
    x = "DPP3 at D3–5",
    y = "ADRB2 at D3–5"
  ) +
  annotate("text", x = min(F3c_dpp3_adrb2$DPP3_D3_5, na.rm = TRUE),
           y = max(F3c_dpp3_adrb2$ADR2_J3_J5, na.rm = TRUE),
           label = paste("Spearman correlation:", F3c_dpp3_adrb2$correlation_D3_5[1]),
           hjust = 0, size = 5, color = "black") +
  theme_minimal()

Fig_DPP3_ADRB2_w <- ggplot(F3c_dpp3_adrb2, aes(x = DPP3_w, y = ADR2_JS)) +
  geom_point(color = "red", alpha = 0.6) +
  geom_smooth(method = "lm", color = "red", se = TRUE) +
  labs(
    title = "Correlation between DPP3 at Weaning and ADRB2 at Weaning",
    x = "DPP3 at Weaning",
    y = "ADRB2 at Weaning"
  ) +
  annotate("text", x = min(F3c_dpp3_adrb2$DPP3_w, na.rm = TRUE),
           y = max(F3c_dpp3_adrb2$ADR2_JS, na.rm = TRUE),
           label = paste("Spearman correlation:", F3c_dpp3_adrb2$correlation_w[1]),
           hjust = 0, size = 5, color = "black") +
  theme_minimal()

print(Fig_DPP3_ADRB2_D0)
print(Fig_DPP3_ADRB2_D3_5)
print(Fig_DPP3_ADRB2_w)

```

# Analysis with monocyte

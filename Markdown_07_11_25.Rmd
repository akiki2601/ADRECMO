...
prep_A1 <- prep_marker_data(df, prefix = "ADR1L", log1p_y = TRUE)
...
# ADRB2 (lymphocytes) — log1p pour modèle & plot
prep_A2 <- prep_marker_data(df, prefix = "ADR2L", log1p_y = TRUE)
...
# ADRB1 (monocytes) — on peut tracer en brut mais le modèle reste sur log1p
prep_M1 <- prep_marker_data(df, prefix = "M_ADR1", log1p_y = TRUE)
...
Figure_1_3 <- plot_marker(
  ...
  panel_title = "C",
  y_var    = "value_log",   # tracé en valeurs brutes si tu préfères
...
# ADRB2 (monocytes)
prep_M2 <- prep_marker_data(df, prefix = "M_ADR2", log1p_y = TRUE)
...
Figure_1_4 <- plot_marker(
  ...
  y_lab    = "Percentage of monocytes expressing ADRB2\n(log1p scale)",
   show_legend = TRUE   # 👈 ici seule cette figure garde la légende
) +
  annotate("text",
           x = 1,

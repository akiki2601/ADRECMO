...
df_list <- lst(ADR1_lympho, ADR2_lympho, ADR1_mono, ADR2_mono)
...
count_valid_ID(df_list)

df <- reduce(df_list, full_join, by = "ID")%>%
...
df_list_1 <- lst(df,DATA)

df_1 <- reduce(df_list_1, left_join, by = "ID")
dim(df_1)
...
# Comparaison des modèles (test de l'interaction)
interact <- anova(mod_no_interaction, mod1)
# Interprétation : p = 0.3758 → interaction non significative

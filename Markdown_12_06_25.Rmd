...
df <- df %>%
  mutate(
    ...
    VIS_JS = (jsev_dobu_cum * 1000) / (Weight * 1440) + 
         100 * (jsev_NAd_cum * 1000) / (Weight * 1440)+ 50 * Levosimendan*0.2,
    ...
    VIS_JS = (jsev_dobu_cum * 1000) / (Weight ) + 
         100 * (jsev_NAd_cum * 1000) / (Weight)+ 50 * Levo_before_ecmo_end*0.2** 1000*Weight,
...
F1_4 <- df %>%
  select(ID,ADR2m_J0, ADR2m_J3_5, ADR2m_JS, Outcome)%>%
  mutate(Outcomes = factor(case_when(Outcome=="Bridge to LVAD"|Outcome=="Bridge to Transplant"~ "Bridge to transplant or LVAD",T~Outcome), levels=c("Death","Bridge to transplant or LVAD","ECMO Weaning")))%>%
  pivot_longer(
...
# Figure 3: Association of cumulative VIS and ADRB in Lymphocyte at day of weaning

```{r, include=F}

# ADRB1
F3_a <- df %>%
  select(VIS_JS, ADR1_JS) %>%
  mutate(VIS_JS_cut = case_when(
    VIS_JS > median(VIS_JS, na.rm = TRUE) ~ "> median",
    VIS_JS <= median(VIS_JS, na.rm = TRUE) ~ "≤ median"
  )) %>%
  filter(!is.na(ADR1_JS), !is.na(VIS_JS))%>%
  mutate(VIS_JS_cut = factor(VIS_JS_cut, levels = c("≤ median", "> median")))


# Test di Mann–Whitney
mw_adr1 <- wilcox.test(log10(ADR1_JS) ~ VIS_JS_cut, data = F3_a)
p_val_adr1 <- sprintf("%.3f", mw_adr1$p.value)

cor_test_adr1 <- cor.test(F3_a$ADR1_JS, F3_a$VIS_JS, method = "spearman")
...
# Figure ADR1
Figure3a <- ggplot(F3_a, aes(x = VIS_JS_cut, y = log10(ADR1_JS), fill = VIS_JS_cut)) +
  geom_boxplot() +
  geom_point(position = position_jitter(width = 0.1), alpha = 0.7, size = 2) +
  geom_text(data = count_data_a,
            aes(x = VIS_JS_cut, y = min(log10(F3_a$ADR1_JS), na.rm = TRUE) * 1.1,
                label = paste0("n=", n)),
            size = 4,
            inherit.aes = FALSE) +
  ...
  labs(
    title = "A",
    x = "Cumulative VIS (cut at median)",
    y = "ADRB1 at ECMO weaning on lymphocytes\n(log10 scale)"
  ) +
  theme_minimal() +
  theme(legend.position = "none") +
  annotate("text",
...
F3_b <- df %>%
  select(VIS_JS, ADR2_JS) %>%
  mutate(VIS_JS_cut = case_when(
    VIS_JS > median(VIS_JS, na.rm = TRUE) ~ "> median",
    VIS_JS <= median(VIS_JS, na.rm = TRUE) ~ "≤ median"
  )) %>%
  filter(!is.na(ADR2_JS), !is.na(VIS_JS))%>%
  mutate(VIS_JS_cut = factor(VIS_JS_cut, levels = c("≤ median", "> median")))

# Test
mw_adr2 <- wilcox.test(log10(ADR2_JS) ~ VIS_JS_cut, data = F3_b)
p_val_adr2 <- sprintf("%.3f", mw_adr2$p.value)

cor_test_adr2 <- cor.test(F3_b$ADR2_JS, F3_b$VIS_JS, method = "spearman")
...
# Figure ADR2
Figure3b <- ggplot(F3_b, aes(x = VIS_JS_cut, y = log10(ADR2_JS), fill = VIS_JS_cut)) +
  geom_boxplot() +
  geom_point(position = position_jitter(width = 0.1), alpha = 0.7, size = 2) +
  geom_text(data = count_data_b,
            aes(x = VIS_JS_cut, y = min(log10(F3_b$ADR2_JS), na.rm = TRUE) * 1.1,
                label = paste0("n=", n)),
            size = 4,
            inherit.aes = FALSE) +
  scale_fill_manual(values = c("≤ median" = "#009E73", "> median" = "#CC79A7")) +
  labs(
    title = "B",
    x = "Cumulative VIS (cut at median)",
    y = "ADRB2 at ECMO weaning on lymphocytes\n(log10 scale)"
  ) +
  theme_minimal() +
  theme(legend.position = "none") +
  annotate("text",
...
# === Combine ===
Figure3_VIS_cum <- Figure3a + Figure3b +
...
# Prepare data
F3_m_a <- df %>%
  select(VIS_JS, ADR1m_JS) %>%
  mutate(
    VIS_JS_cut = case_when(
      VIS_JS > median(VIS_JS, na.rm = TRUE) ~ "> median",
      TRUE ~ "≤ median"
    )
  ) %>%
  filter(!is.na(ADR1m_JS), !is.na(VIS_JS)) %>%
  mutate(VIS_JS_cut = factor(VIS_JS_cut, levels = c("≤ median", "> median")))


# Mann–Whitney test
mw_test_adr1m <- wilcox.test(log10(ADR1m_JS) ~ VIS_JS_cut, data = F3_m_a)
p_value_adr1m <- sprintf("%.3f", mw_test_adr1m$p.value)

cor_test_adr1m <- cor.test(F3_m_a$ADR1m_JS, F3_m_a$VIS_JS, method = "spearman")
...
# Plot
Figure3_VIS_cum_a <- ggplot(F3_m_a, aes(x = VIS_JS_cut, y = log10(ADR1m_JS), fill = VIS_JS_cut)) +
  geom_boxplot() +
  geom_point(position = position_jitter(width = 0.1), alpha = 0.7, size = 2) +
  geom_text(
    data = F3_m_a %>% group_by(VIS_JS_cut) %>% summarise(n = n()) %>%
      mutate(y_n_label = -2.5),  
    aes(x = VIS_JS_cut, y = y_n_label, label = paste0("n=", n)),
    inherit.aes = FALSE, size = 4
  ) +
  scale_fill_manual(values = c("≤ median" = "#009E73", "> median" = "#CC79A7")) +
  labs(title = "A", x = "Cumulative VIS", y = "ADRB1 in monocytes at weaning\n (log10)") +
  theme_minimal() +
  theme(legend.position = "none") +
  annotate("text",
           x = 1.5,
...
F3_m_b <- df %>%
  ...
  filter(!is.na(ADR2m_JS), !is.na(VIS_JS))%>%
  mutate(VIS_JS_cut = factor(VIS_JS_cut, levels = c("≤ median", "> median")))

# Mann–Whitney test
mw_test_adr2m <- wilcox.test(log10(ADR2m_JS) ~ VIS_JS_cut, data = F3_m_b)
p_value_adr2m <- sprintf("%.3f", mw_test_adr2m$p.value)

cor_test_adr2m <- cor.test(F3_m_b$ADR2m_JS, F3_m_b$VIS_JS, method = "spearman")
...
# Plot
Figure3_VIS_cum_b <- ggplot(F3_m_b, aes(x = VIS_JS_cut, y = log10(ADR2m_JS), fill = VIS_JS_cut)) +
  geom_boxplot() +
  geom_point(position = position_jitter(width = 0.1), alpha = 0.7, size = 2) +
  geom_text(data = count_data_adr2m, aes(x = VIS_JS_cut, y = min(log10(F3_m_b$ADR2m_JS), na.rm = TRUE) * 1.25,
                                         label = paste0("n=", n)), inherit.aes = FALSE, size = 4) +
  scale_fill_manual(values = c("≤ median" = "#009E73", "> median" = "#CC79A7")) +
  labs(title = "B", x = "Cumulative VIS", y = "ADRB2 in monocytes at weaning\n(log10)") +
  theme_minimal() +
  theme(legend.position = "none") +
  annotate("text", x = 1.5, y = max(log10(F3_m_b$ADR2m_JS), na.rm = TRUE), label = paste("p:", p_value_adr2m), size = 5)

# Cumulative
Figure3m_VIS_cum <- Figure3_VIS_cum_a + Figure3_VIS_cum_b +
  plot_annotation(title = "Association between cumulative VIS and ADR expression in monocytes")

...
df_hapto <- read_excel("DATA/hapto.xlsx", sheet="ADRECMO ")%>%
  select(`Sample ID number`,
         `Haptoglobin\r\ng/l\r\nSiemes Atellica`)%>%
  rename(ID_pre=`Sample ID number`,
         hapto=`Haptoglobin\r\ng/l\r\nSiemes Atellica`)%>%
  mutate(
  ID_num = str_replace(ID_pre, ".*-(\\d{2})_([A-Za-z]+).*", "\\1"),
  time_points = str_replace(ID_pre, "^[^_]*_[^_]*_",""),
  hapto = case_when(hapto=="<LDL" ~ "0",
                    TRUE ~hapto),
  ID_num=as.numeric(ID_num))%>%
    select(ID_num,
           ...
           hapto)%>%
  left_join(df_ids%>%
              mutate(ID_num=as.numeric(ID_num)),
            by = "ID_num")%>%
  select(-ID_num)%>%
  filter(!is.na(ID))%>%
  mutate(time_points = dplyr::case_when(
      time_points %in% c("JSEVRAGE", "ADRECMO-40_JS") ~ "JS",
      time_points %in% c("J3", "J4", "J5","J3-5") ~ "J3-J5",
      TRUE ~ time_points)
      )%>%
    pivot_wider(names_from = time_points,
                values_from = hapto,
                 names_prefix = "hapto_")


df_fv <- df_fv %>% left_join(df_hapto, by = "ID")
```
...
df <- df_fv %>%
  mutate(
    ...
    Outcome_death_bridge = case_when(
      Outcome %in% c("Bridge to Transplant", "Bridge to LVAD", "Death") ~ "Death or Bridge",
      TRUE ~ as.character(Outcome)
    ),
    ...
    NAD_jp2_w = jp2_NAd_cum*1000/Weight,
    ...
    DPP3_group_JS    = factor(ifelse(DPP3_w     > 40, 1, 0), labels = c("DPP3 ≤ 40", "DPP3 > 40")),
     DPP3_median = as.factor(if_else(df$DPP3_D0<=median(df$DPP3_D0), "below median", "above median")),
    ...
    BCPIA_Tot = as.integer(pmax(IABP_D0, jp2_cpbia, jsev_cpbia, na.rm = TRUE) >= 1),
    EER_Tot = as.integer(pmax(EER_D0, jp2_eer, jsev_eer, na.rm = TRUE) >= 1),
...
saveRDS(df, file = "DATA/df_managed.rds")
```
...
summary_stats <- df %>%
  ...
  summarise(
    mean_hours = mean(time_diff_hours),
    sd_hours = sd(time_diff_hours),
    min = min(time_diff_hours),
    max = max(time_diff_hours)
  )
```

Day-0 blood samples were collected on average `r round(summary_stats$mean_hours, 2)` hours (SD `r round(summary_stats$sd_hours, 2)` hours) before ECMO implantation, ranging from `r round(summary_stats$min, 1)` hours before to `r round(summary_stats$min, 1)` hours after ECMO cannulation.
...
var.cont <- c("Age", "BMI", "PAS_D0", "PAD_D0", "PAM_D0", "HR_D0", "Preecmo_tte_ef", "Preecmo_tte_vtiao", "NAD_24H", "DOB_24H", "Lact_D0", "Creat_D0", "Bili_D0", "Tropo_i_hs_D0", "Ntprobnp_D0",  "Plq_D0")
...
traduction <- c(
  ...
  "NAD_24H" = "Norepinephrine cumulative Dose on the VA-ECMO implantation day (γ/kg)",
  "DOB_24H" = "Dobutamine cumulative Dose on the VA-ECMO implantation day (γ/kg)",
...
order <- c(
  ...
  "Norepinephrine cumulative Dose on the VA-ECMO implantation day (γ/kg)",
  "Dobutamine cumulative Dose on the VA-ECMO implantation day (γ/kg)",
...
rows_to_round <- table_1_vf2$Variables %in% zerodigit

table_1_vf2$Overall[rows_to_round] <-
  round_nonnormal_0(table_1_vf2$Overall[rows_to_round])


table1 <- flextable(table_1_vf2) %>%
  set_caption("Table 1: Characteristics of the population at VA-ECMO implantation") %>% 
  autofit() %>%
...
var.cont = c("Lenght_vm",
             "Time_hosp",
             "Time_ecmo",
             "ECMO_duration_weaned",
             "ECMO_duration_Dead")

var.cat = c("BCPIA_Tot", "EER_Tot" ,"Outcome","j90_deces")
...
table_2_vf$Overall <- round_nonnormal_0(table_2_vf$Overall)


# 4) Application traduction + ordre + renommage Missing
table_2_vf <- table_2_vf %>%
  mutate(    across(everything(), ~gsub("\\[", "(", .x )),
    across(everything(), ~gsub("\\]", ")", .x )),
    across(everything(), ~gsub("\\s*[;,]\\s*", " – ", .x)),
...
table2 <- flextable(table_2_vf) %>%
  set_caption("Table 2: Outcomes in the whole population") %>% 
  autofit() %>%
...
```{r FIGURE_1BIS, include=FALSE}
F1bis_1_plot <- make_il_cor_plot(
  df,
  "ADR1L_J0", "log_value_IL10_J0",
  xlab = "log(IL-10 + 1)",
  ylab = "Percentage of T lymphocytes\nexpressing ADRB1",
  title_lab = "A: implantation",
  ylim = c(0, 100)
...
F1bis_2_plot <- make_il_cor_plot(
  df,
  "ADR1L_J3_J5", "log_value_IL10_J3_J5",
  xlab = "log(IL-10 + 1)",
  ylab = "Percentage of T lymphocytes\nexpressing ADRB1",
  title_lab = "B: D3–D5",
  ylim = c(0, 100)
...
F1bis_3_plot <- make_il_cor_plot(
  df,
  "ADR1L_JS", "log_value_IL10_JS",
  xlab = "log(IL-10 + 1)",
  ylab = "Percentage of T lymphocytes\nexpressing ADRB1",
  title_lab = "C: explantation",
  ylim = c(0, 100)
...
F1bis_4_plot <- make_il_cor_plot(
  df,
  "ADR2L_J0", "log_value_IL10_J0",
  xlab = "log(IL-10 + 1)",
  ylab = "Percentage of T lymphocytes\nexpressing ADRB2",
  title_lab = "D: implantation",
  ylim = c(0, 100)
...
F1bis_5_plot <- make_il_cor_plot(
  df,
  "ADR2L_J3_J5", "log_value_IL10_J3_J5",
  xlab = "log(IL-10 + 1)",
  ylab = "Percentage of T lymphocytes\nexpressing ADRB2",
  title_lab = "E: D3–D5",
  ylim = c(0, 100)
...
F1bis_6_plot <- make_il_cor_plot(
  df,
  "ADR2L_JS", "log_value_IL10_JS",
  xlab = "log(IL-10 + 1)",
  ylab = "Percentage of T lymphocytes\nexpressing ADRB2",
  title_lab = "F: explantation",
  ylim = c(0, 100)
...
Figure_1_bis <- (F1bis_1_plot | F1bis_2_plot | F1bis_3_plot) /
                (F1bis_4_plot | F1bis_5_plot | F1bis_6_plot)
...
```{r Figure4-display, fig.width=8, fig.height=6, fig.scale=1.5, dpi=300, fig.align='center'}

library(patchwork)
Figure4 <- (Figure4_A) | (Figure4_B) +
...
```{r FigureS1-display, fig.width=8, fig.height=6, fig.scale=1.5, dpi=300, fig.align='center'}
FigureS1

...
#Verif
verif <- readxl::read_excel("DATA/ADRECMO_Verif.xlsx")%>%
   mutate(Initiales = sub("-","",Initiales),
     ID = paste0(`N° patient`,"-",Initiales),
     ID = str_remove(ID, "^0+"))%>%
   rename(cause= `Origine du choc cardiogénique`)%>%
   dplyr::select(ID,cause,CA)

df <- df%>% left_join(verif, by="ID") 
table(is.na(df$cause))
...
df_bio <- df %>%
  select(
    ...
    starts_with("mixte_"),
    starts_with("bINFg"),
    starts_with("IL"),          # toutes les cytokines IL…
    starts_with("IFNg"),
    starts_with("TNF"),
    starts_with("GDF-15"),
  ...
  mutate(   Outcome = case_when(
      ...
      Outcome == 2 ~ "Bridge to Transplant or LVAD",
      Outcome == 3 ~ "Bridge to Transplant or LVAD",
      Outcome == 4 ~ "Death",
...
var.cont <- c("Age", "BMI", "PAS_D0", "PAD_D0", "PAM_D0", "Pulsepressure_D0", "HR_D0", "Temp_D0", "Preecmo_tte_ef", "Preecmo_tte_vtiao", "NADcum_D0", "DOBUcum_D0", "pH_D0", "Sao2_D0", "Paco2_D0", "Lact_D0", "Hco3_D0", "Creat_D0", "Urea_D0", "ALAT_D0", "ASAT_D0", "Bili_D0", "PT_D0", "Tropo_i_hs_D0", "Ntprobnp_D0", "DPP3_D0", "Plq_D0", "Lenght_eer", "Lenght_vm", "Time_hosp", "Time_ecmo")
...
######
vars_to_plot <- c("ADR1L_J0", "ADR1L_J3_J5","ADR1L_JS")
plot_density_log10(df, vars_to_plot, breaks=30)

res <- run_th1th2_analysis(
  df,
  time_cols = c("ADR1L_J0", "ADR1L_J3_J5","ADR1L_JS")
)

vars_to_plot <- c("ADR2L_J0", "ADR2L_J3_J5","ADR2L_JS")
plot_density_log10(df, vars_to_plot, breaks=30)

res <- run_th1th2_analysis(
  df,
  time_cols = c("ADR2L_J0", "ADR2L_J3_J5","ADR2L_JS")
)

res <- run_th1th2_analysis(
  df,
  time_cols = c("L_TH2_J0", "L_TH2_J3_J5","L_TH2_JS")
)
...
res <- run_th1th2_analysis(
  df,
  time_cols = c("M_ADR1_J0", "M_ADR1_J3_J5","M_ADR1_JS")
)

res <- run_th1th2_analysis(
  df,
  time_cols = c("M_ADR2_J0", "M_ADR2_J3_J5","M_ADR2_JS")
)
...
F1_2 <- df %>%
  ...
  mutate(time = as.numeric(case_when(ADR2l_time=="ADR2l_J0"~1,
                          ADR2l_time=="ADR2l_J3_J5"~2,
                          ADR2l_time=="ADR2l_JS"~3)),
         value_log = log10(value))


count_data <- F1_2 %>%
  filter(!is.na(value))%>%
  group_by(ADR2l_time, Outcomes) %>%
  summarise(n = n(), .groups = "drop")%>%
  filter(!is.na(Outcomes))
...
Figure_1_2 <- ggplot(F1_2, aes(x = ADR2l_time, y = value_log, fill = Outcomes)) +
  geom_boxplot(position = position_dodge(0.7), width = 0.6) +  
  scale_y_log10() +  
  labs(
    title = "B",
  ...
  geom_text(data = count_data, aes(x = ADR2l_time, y = -min(F1_2$value_log, na.rm = TRUE) * 0.01, 
                                   label = paste0("n=", n), group = Outcomes), 
            position = position_dodge(0.7), size = 4, vjust = 1)+
